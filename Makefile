...
deploy-token-testnet: 
	@forge script script/deployment/DeployERC20Token.s.sol:DeployERC20Token --rpc-url $(RPC_BSC_TEST) --account <account name> --sender <account addres> --broadcast --verify --etherscan-api-key $(BSCSCAN_KEY)
deploy-source-testnet: 
	@forge script script/deployment/DeploySourceMinter.s.sol:DeploySourceMinter --rpc-url $(RPC_BSC_TEST) --account <account name> --sender <account addres> --broadcast --verify --etherscan-api-key $(BSCSCAN_KEY)
deploy-destination-testnet: 
	@forge script script/deployment/DeployDestinationMinter.s.sol:DeployDestinationMinter --rpc-url $(RPC_BASE_SEPOLIA) --account <account name> --sender <account addres> --broadcast --verify --etherscan-api-key $(BASESCAN_SEPOLIA_KEY) -vvvv

deploy-source-mainnet: 
	@forge script script/deployment/DeploySourceMinter.s.sol:DeploySourceMinter --rpc-url $(RPC_BSC_MAIN) --account <account name> --sender <account addres> --broadcast --verify --etherscan-api-key $(BSCSCAN_KEY)
deploy-destination-mainnet: 
	@forge script script/deployment/DeployDestinationMinter.s.sol:DeployDestinationMinter --rpc-url $(RPC_BASE_MAIN) --account <account name> --sender <account addres> --broadcast --verify --etherscan-api-key $(BASESCAN_KEY) -vvvv
